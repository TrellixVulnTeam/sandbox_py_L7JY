...
class Pymodoro:
    def __init__(self, master):
        self.master = master
        self.mainframe = tkinter.Frame(self.master, bg='white')  # 継承するパターンもある
    ...
    def update(self):
        ...
        if self.running and time_left > 0:
            self.draw_time(time_left)
            self.time_left.set(time_left - 1)
        else:
            self.draw_time(DEFAULT_GAP)
            self.stop_timer()
        self.master.after(1000, self.update)  # recursive

    def draw_time(self, time):
        minutes, seconds = self.minutes_seconds(time)
