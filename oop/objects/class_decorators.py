...
def invariant(predicate):
    ...
    def invariant_checking_class_decorator(cls):
        ...
        property_names = [name for name, attr in vars(cls).items()
...
class InvariantCheckingPropertyProxy:
    ...
    def __get__(self, instance, owner):
        ...
        if not self._predicate(instance):
            raise RuntimeError(
                "Class invariant {!r} violated for {!r}".format(self._predicate.__doc__, instance))
        return result
...
if __name__ == '__main__':
    ...
    try:
        t.celsius = -300  # Error ok
    except RuntimeError as e:
        print(e)

    try:
        t.celsius = 1e34  # no Error but NG : compare to abc.class_decorators
    except RuntimeError as e:
        print(e)
