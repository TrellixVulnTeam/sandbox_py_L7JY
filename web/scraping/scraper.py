...
def process_car_blocks(soup):
    ...
    with open("../../tmp/scraped_cars.csv", "w", newline='\n') as f:
...
def extract_territory_year(cb):
    ...
    assert len(territory) > 1,\
        f"Expecting territory to be a useful string not {territory}"
...
def extract_horsepower(cb):
    ...
    try:
        ...
        hp = "NULL"  # workaround for missing value
    return hp
...
def extract_mpg(cb):
    ...
    try:
        ...
        mpg = "NULL"  # workaround for missing value
    return mpg
...
def extract_displacement(text):
    ...
    assert displacement > 60,\
        f"Expecting a reasonable displacement, not {displacement}"
    return displacement


# python -m http.server
# PAGE = "http://localhost:8000/auto_mpg.html"
PAGE = 'http://localhost/home/notebooks/scraping/data/auto_mpg.html'

if __name__ == "__main__":
    filename = '../../tmp/scraped_page_result.pickle'
    if os.path.exists(filename):
        ...
        assert result.status_code == 200,\
            f"Got status code {result.status_code} which isn't a success"
